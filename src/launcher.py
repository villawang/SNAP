...
* The module to be launched (and various other options) can be specified at the command line;
  ...
  launcher.py --module Sample1 --studypath studies/Sample1 --autolaunch 1 --developer
  1 --engineconfig defaultsettings.prc --datariver 0 --labstreaming 1 --fullscreen 0 --
  windowsize 800x600 --windoworigin 50/50 --noborder 0 --nomousecursor 0 --timecompensation 1
...
* In addition to modules, there are "study configuration files" (aka study configs),
  ...
  A config can be specified in the command line just by passing the appropriate .cfg file name, as in the following
  example.
...
* The program can be remote-controlled via a simple TCP text-format network protocol (on port 7899) supporting the
  following messages:
  ...
  config configname.cfg  --> load a config named configname.cfg (make sure that the studypath is set correctly
  so that it's found)
...
# Set this to a pixel offset from left top corner, e.g. "50/50" (with quotes) to override the window location in the
# engineconfig file
WINDOWORIGIN = None
...
loadPrcFileData('', 'model-path ' + opts.studypath + '/trial1')
loadPrcFileData('', 'model-path ' + opts.studypath)
loadPrcFileData('', 'model-path trial1')

loadPrcFileData('', 'model-path ' + opts.studypath + '/trial2')
loadPrcFileData('', 'model-path ' + opts.studypath)
loadPrcFileData('', 'model-path trial2')
